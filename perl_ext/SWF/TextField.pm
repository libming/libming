...
the hex or dezimal notation. Even mixed.
...
Set the height of font in your texfield to 14 pixels.
...
Two textfields. First shows a changing actionscriptvariable, in the 
second we put a Perlvariable as string into the textfield.
	...
	$f = new SWF::Font("_sans.fdb"); #good, _sans is a good default font. 
        ...
        $tf1->setBounds(70,20); # default value: ??
        $tf1->setName("_root.currentFrame"); # Actionscript variable	

	$tf2 = new SWF::TextField(SWFTEXTFIELD_DRAWBOX | SWFTEXTFIELD_NOEDIT);
	$tf2->setFont($f);
	$tf2->setHeight(11.25); # font size, default value: ??
	$tf2->setBounds(50,14); # default value: ??
	$tf2->addString($OSNAME); # put the systems name into the field

	# put our 2 textfield in the movie
	$tf1_i= $m->add($tf1);
	$tf2_i= $m->add($tf2);
	$tf1_i->move(20,20);
	$tf2_i->move(20,50);
...
Soheil Seyfaie (soheil@users.sourceforge.net)

Documentation: Peter Liscovius (peterdd@users.sourceforge.net)

=head1 SEE ALSO

	SWF:Constants, SWF::Font, SWF::Movie
