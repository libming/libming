...
class SWFBlock
{
 public:
  ...
  boolean isCharacter()
  {
    c_SWFBlock block = this->getBlock();

    if(block == NULL)
      return false;

    return SWFBlock_isCharacter(block) ? true : false;
  }
  */
};
...
class SWFRect
{
 public:
  ...
  SWFRect(int minX, int maxX, int minY, int maxY)
    { this->rect = newSWFRect(minX, maxX, minY, maxY); }

  SWFRect(c_SWFRect rect)
    { this->rect = rect; }

  virtual ~SWFRect()
    { destroySWFRect(this->rect); }

  int getWidth()
    { return SWFRect_getWidth(this->rect); }

  int getHeight()
    { return SWFRect_getHeight(this->rect); }
};
...
class SWFCharacter : public SWFBlock
{
 public:
  ...
  SWFRect getBounds()
    { return new SWFRect(SWFCharacter_getBounds(this->character)); }
...
class SWFShape : public SWFCharacter
{
 public:
  ...
  void movePen(int x, int y)
    { SWFShape_moveToRelative(this->shape, x, y); }

  void movePenTo(int x, int y)
    { SWFShape_moveTo(this->shape, x, y); }

  void drawLine(int x, int y)
    { SWFShape_lineToRelative(this->shape, x, y); }

  void drawLineTo(int x, int y)
    { SWFShape_lineTo(this->shape, x, y); }

  void drawCurve(int cx, int cy, int ax, int ay)
    { SWFShape_curveToRelative(this->shape, cx, cy, ax, ay); }

  void drawCurveTo(int cx, int cy, int ax, int ay)
    { SWFShape_curveTo(this->shape, cx, cy, ax, ay); }
