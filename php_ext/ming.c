 ...
 */
static void *
SWFgetProperty (zval * id, char *name, int namelen, int proptype TSRMLS_DC)
{
  zval **tmp;
  int id_to_find;
  void *property;
...
static SWFCharacter
getCharacter (zval * id TSRMLS_DC)
{
...
*/
static void
destroy_SWFInput_resource (zend_rsrc_list_entry * resource TSRMLS_DC)
{
  destroySWFInput ((SWFInput) resource->ptr);
}

static SWFInput
getInput (zval ** zfile TSRMLS_DC)
{
  FILE *file;
  void *what;
  int type;
  ...
  what =
    zend_fetch_resource (zfile TSRMLS_CC, -1, "File-Handle", &type, 1,
			 php_file_le_stream ());

  if (php_stream_cast
      ((php_stream *) what, PHP_STREAM_AS_STDIO, (void *) &file,
...
static zend_function_entry swfaction_functions[] = {
  PHP_FALIAS (swfaction, swfaction_init, NULL) {NULL, NULL, NULL}
};
...
static zend_function_entry swfbitmap_functions[] = {
  PHP_FALIAS (swfbitmap, swfbitmap_init, NULL)
    ...
    PHP_FALIAS (getheight, swfbitmap_getHeight, NULL) {NULL, NULL, NULL}
};
...
PHP_FUNCTION (swfbitmap_init)
{
  ...
  if (zmask != NULL)
  {
    if (Z_TYPE_PP (zmask) != IS_RESOURCE)
    {
      ...
      maskinput =
	newSWFInput_buffer (Z_STRVAL_PP (zmask), Z_STRLEN_PP (zmask));
...
static void
destroy_SWFBitmap_resource (zend_rsrc_list_entry * resource TSRMLS_DC)
{
...
static void
destroy_SWFButton_resource (zend_rsrc_list_entry * resource TSRMLS_DC)
{
...
/* {{{ internal function getSound
   Returns the SWFSound object contained in zval *id */

static SWFSound getSound(zval *id TSRMLS_DC)
{
  void *sound = SWFgetProperty(id, "sound", 4, le_swfsoundp TSRMLS_CC);
...
/* {{{ proto void swfbutton_addSound(SWFSound sound, int flags)
   Sets the sound to play for the condition described in flags */
PHP_FUNCTION (swfbutton_addSound)
{
  ...
  convert_to_long_ex (flags);
  SWFButton_addSound (button, sound, Z_LVAL_PP (flags));
}

static void destroy_SWFSound_resource(zend_rsrc_list_entry *resource)
{
  destroySWFSound((SWFSound)resource->ptr);
}
...
PHP_FUNCTION (swfbutton_setAction)
{
  ...
  if (ZEND_NUM_ARGS () != 1
      || zend_get_parameters_ex (1, &zaction) == FAILURE)
  {
    WRONG_PARAM_COUNT;
  }
...
static zend_function_entry swfdisplayitem_functions[] = {
  PHP_FALIAS (moveto, swfdisplayitem_moveTo, NULL)
    ...
    PHP_FALIAS (addaction, swfdisplayitem_addAction, NULL) {NULL, NULL, NULL}
};

/* {{{ internal function getDisplayItem
   Returns the SWFDisplayItem contained in zval *id */
static SWFDisplayItem
getDisplayItem (zval * id TSRMLS_DC)
{
  void *item =
    SWFgetProperty (id, "displayitem", 11, le_swfdisplayitemp TSRMLS_CC);
...
PHP_FUNCTION (swfdisplayitem_rotateTo)
{
  ...
  if (ZEND_NUM_ARGS () != 1
      || zend_get_parameters_ex (1, &degrees) == FAILURE)
  {
    WRONG_PARAM_COUNT;
  }
...
PHP_FUNCTION (swfdisplayitem_setDepth)
{
  ...
  SWFDisplayItem_setDepth (getDisplayItem (getThis ()TSRMLS_CC),
			   Z_LVAL_PP (depth));
}
...
PHP_FUNCTION (swfdisplayitem_multColor)
{
  ...
  SWFDisplayItem_setColorMult (getDisplayItem (getThis ()TSRMLS_CC),
			       Z_DVAL_PP (r), Z_DVAL_PP (g), Z_DVAL_PP (b),
			       a);
}
...
static zend_function_entry swffill_functions[] = {
  PHP_FALIAS (swffill, swffill_init, NULL)
    ...
    PHP_FALIAS (skewyto, swffill_skewYTo, NULL) {NULL, NULL, NULL}
};
...
static void
destroy_SWFFill_resource (zend_rsrc_list_entry * resource TSRMLS_DC)
{
...
PHP_FUNCTION (swffill_moveTo)
{
  ...
  convert_to_double_ex (x);
  convert_to_double_ex (y);
  SWFFill_moveTo (getFill (getThis ()TSRMLS_CC), Z_DVAL_PP (x),
		  Z_DVAL_PP (y));
}
...
static zend_function_entry swffont_functions[] = {
  PHP_FALIAS (swffont, swffont_init, NULL)
    ...
    PHP_FALIAS (getleading, swffont_getLeading, NULL) {NULL, NULL, NULL}
};

/* {{{ internal function SWFText getFont(zval *id)
   Returns the Font object in zval *id */
static SWFFont
getFont (zval * id TSRMLS_DC)
{
...
PHP_FUNCTION (swffont_init)
{
  ...
  if (strcmp (Z_STRVAL_PP (zfile) + Z_STRLEN_PP (zfile) - 4, ".fdb") == 0)
  {
    ...
    stream =
      php_stream_open_wrapper (Z_STRVAL_PP (zfile), "rb",
    ...
    if (FAILURE ==
	php_stream_cast (stream, PHP_STREAM_AS_STDIO, (void *) &file,
			 REPORT_ERRORS))
    {
...
static void
destroy_SWFFont_resource (zend_rsrc_list_entry * resource TSRMLS_DC)
{
...
PHP_FUNCTION (swffont_getWidth)
{
  ...
  if (ZEND_NUM_ARGS () != 1
      || zend_get_parameters_ex (1, &zstring) == FAILURE)
  {
    WRONG_PARAM_COUNT;
  }
  convert_to_string_ex (zstring);
  width =
    SWFFont_getStringWidth (getFont (getThis ()TSRMLS_CC),
			    Z_STRVAL_PP (zstring));
  RETURN_DOUBLE (width);
}
...
static zend_function_entry swfgradient_functions[] = {
  PHP_FALIAS (swfgradient, swfgradient_init, NULL)
    PHP_FALIAS (addentry, swfgradient_addEntry, NULL) {NULL, NULL, NULL}
};
...
static void
destroy_SWFGradient_resource (zend_rsrc_list_entry * resource TSRMLS_DC)
{
...
static SWFGradient
getGradient (zval * id TSRMLS_DC)
{
  void *gradient =
    SWFgetProperty (id, "gradient", 8, le_swfgradientp TSRMLS_CC);
...
PHP_FUNCTION (swfgradient_addEntry)
{
  ...
  SWFGradient_addEntry (getGradient (getThis ()TSRMLS_CC),
			Z_DVAL_PP (ratio), Z_LVAL_PP (r), Z_LVAL_PP (g),
			Z_LVAL_PP (b), a);
}
...
static zend_function_entry swfmorph_functions[] = {
  PHP_FALIAS (swfmorph, swfmorph_init, NULL)
    ...
    PHP_FALIAS (getshape2, swfmorph_getShape2, NULL) {NULL, NULL, NULL}
};
...
static void
destroy_SWFMorph_resource (zend_rsrc_list_entry * resource TSRMLS_DC)
{
...
static void
destroy_SWFMovie_resource (zend_rsrc_list_entry * resource TSRMLS_DC)
{
...
*/
static void
phpByteOutputMethod (byte b, void *data)
{
  TSRMLS_FETCH ();
...
*/
static void
phpStreamOutputMethod (byte b, void *data)
{
  TSRMLS_FETCH ();
...
PHP_FUNCTION (swfmovie_saveToFile)
{
  ...
  ZEND_FETCH_RESOURCE (what, php_stream *, x, -1, "File-Handle",
		       php_file_le_stream ());
...
PHP_FUNCTION (swfmovie_save)
{
  ...
  {
    ZEND_FETCH_RESOURCE (stream, php_stream *, x, -1, "File-Handle",
			 php_file_le_stream ());
    RETURN_LONG (SWFMovie_output
		 (getMovie (getThis ()TSRMLS_CC), &phpStreamOutputMethod,
		  stream));
  }

  convert_to_string_ex (x);
  stream =
    php_stream_open_wrapper (Z_STRVAL_PP (x), "wb",
			     REPORT_ERRORS | ENFORCE_SAFE_MODE, NULL);
  ...
  retval =
    SWFMovie_output (getMovie (getThis ()TSRMLS_CC), &phpStreamOutputMethod,
		     (void *) stream);
  php_stream_close (stream);
  RETURN_LONG (retval);
}
...
PHP_FUNCTION (swfmovie_setBackground)
{
  ...
  if (ZEND_NUM_ARGS () != 3
      || zend_get_parameters_ex (3, &r, &g, &b) == FAILURE)
  {
    WRONG_PARAM_COUNT;
  }
  convert_to_long_ex (r);
...
PHP_FUNCTION (swfmovie_streamMp3)
{
  ...
  soundstream = newSWFSoundStream_fromInput (input);
  SWFMovie_setSoundStream (movie, soundstream);
}
...
static zend_function_entry swfshape_functions[] = {
  ...
  PHP_FALIAS (drawcubicto, swfshape_drawcubicto, NULL) {NULL, NULL, NULL}
};
/* }}} */
...
static void
destroy_SWFShape_resource (zend_rsrc_list_entry * resource TSRMLS_DC)
{
...
PHP_FUNCTION (swfshape_addfill)
{
  ...
  if (ZEND_NUM_ARGS () == 1 || ZEND_NUM_ARGS () == 2)
  {
	...
	flags = SWFFILL_LINEAR_GRADIENT;
      ...
      fill =
	SWFShape_addGradientFill (getShape (getThis ()TSRMLS_CC),
	...
	flags = SWFFILL_TILED_BITMAP;
      ...
      fill =
	SWFShape_addBitmapFill (getShape (getThis ()TSRMLS_CC),
				getBitmap (*arg1 TSRMLS_CC), flags);
    }
    else
    {
  ...
  else if (ZEND_NUM_ARGS () == 3 || ZEND_NUM_ARGS () == 4)
  {
    ...
    fill =
      SWFShape_addSolidFill (getShape (getThis ()TSRMLS_CC), Z_LVAL_PP (r),
			     Z_LVAL_PP (g), Z_LVAL_PP (b), a);
  }
  else
  {
    WRONG_PARAM_COUNT;
...
PHP_FUNCTION (swfshape_movepento)
{
  ...
  convert_to_double_ex (x);
  convert_to_double_ex (y);
  SWFShape_movePenTo (getShape (getThis ()TSRMLS_CC), Z_DVAL_PP (x),
		      Z_DVAL_PP (y));
}
...
PHP_FUNCTION (swfshape_movepen)
{
  ...
  convert_to_double_ex (x);
  convert_to_double_ex (y);
  SWFShape_movePen (getShape (getThis ()TSRMLS_CC), Z_DVAL_PP (x),
		    Z_DVAL_PP (y));
}
...
PHP_FUNCTION (swfshape_drawlineto)
{
  ...
  convert_to_double_ex (x);
  convert_to_double_ex (y);
  SWFShape_drawLineTo (getShape (getThis ()TSRMLS_CC), Z_DVAL_PP (x),
		       Z_DVAL_PP (y));
}
...
PHP_FUNCTION (swfshape_drawline)
{
  ...
  convert_to_double_ex (x);
  convert_to_double_ex (y);
  SWFShape_drawLine (getShape (getThis ()TSRMLS_CC), Z_DVAL_PP (x),
		     Z_DVAL_PP (y));
}
...
PHP_FUNCTION (swfshape_drawcurveto)
{
  if (ZEND_NUM_ARGS () == 4)
  {
    ...
    SWFShape_drawCurveTo (getShape (getThis ()TSRMLS_CC),
			  Z_DVAL_PP (cx), Z_DVAL_PP (cy), Z_DVAL_PP (ax),
			  Z_DVAL_PP (ay));
...
PHP_FUNCTION (swfshape_drawglyph)
{
  ...
  SWFShape_drawSizedGlyph (getShape (getThis ()TSRMLS_CC),
			   getFont (*font TSRMLS_CC), Z_STRVAL_PP (c)[0],
			   size);
}
...
PHP_FUNCTION (swfshape_drawarc)
{
  ...
  SWFShape_drawArc (getShape (getThis ()TSRMLS_CC), Z_DVAL_PP (r),
		    Z_DVAL_PP (start) * M_PI / 180,
		    Z_DVAL_PP (end) * M_PI / 180);
}
...
static zend_function_entry swfsprite_functions[] = {
  PHP_FALIAS (swfsprite, swfsprite_init, NULL)
    ...
    PHP_FALIAS (setframes, swfsprite_setFrames, NULL) {NULL, NULL, NULL}
};
...
static void
destroy_SWFSprite_resource (zend_rsrc_list_entry * resource TSRMLS_DC)
{
...
PHP_FUNCTION (swfsprite_labelFrame)
{
  ...
  convert_to_string_ex (label);
  SWFMovieClip_labelFrame (getSprite (getThis ()TSRMLS_CC),
			   Z_STRVAL_PP (label));
}
...
static zend_function_entry swftext_functions[] = {
  PHP_FALIAS (swftext, swftext_init, NULL)
    ...
    PHP_FALIAS (getleading, swftext_getLeading, NULL) {NULL, NULL, NULL}
};
...
static void
destroy_SWFText_resource (zend_rsrc_list_entry * resource TSRMLS_DC)
{
...
PHP_FUNCTION (swftext_setSpacing)
{
  ...
  if (ZEND_NUM_ARGS () != 1
      || zend_get_parameters_ex (1, &spacing) == FAILURE)
  {
    WRONG_PARAM_COUNT;
  }
...
PHP_FUNCTION (swftext_getWidth)
{
  ...
  convert_to_string_ex (zstring);
  width =
    SWFText_getStringWidth (getText (getThis ()TSRMLS_CC),
			    Z_STRVAL_PP (zstring));
  RETURN_DOUBLE (width);
}
...
static zend_function_entry swftextfield_functions[] = {
  PHP_FALIAS (swftextfield, swftextfield_init, NULL)
    ...
    PHP_FALIAS (addstring, swftextfield_addString, NULL) {NULL, NULL, NULL}
};
...
static void
destroy_SWFTextField_resource (zend_rsrc_list_entry * resource TSRMLS_DC)
{
...
static SWFTextField
getTextField (zval * id TSRMLS_DC)
{
  void *field =
    SWFgetProperty (id, "textfield", 9, le_swftextfieldp TSRMLS_CC);
...
PHP_FUNCTION (swftextfield_setBounds)
{
  ...
  if (ZEND_NUM_ARGS () != 2
      || zend_get_parameters_ex (2, &width, &height) == FAILURE)
  {
    WRONG_PARAM_COUNT;
  }
...
PHP_FUNCTION (swftextfield_setColor)
{
  ...
  convert_to_long_ex (g);
  convert_to_long_ex (b);
  SWFTextField_setColor (field, Z_LVAL_PP (r), Z_LVAL_PP (g), Z_LVAL_PP (b),
			 alpha);
}
  ...
  PHP_MINFO_FUNCTION (ming)
{
  ...
  php_info_print_table_row (2, "Ming SWF output library",
			    "the funk in your trunk");
...
#define CONSTANT(s,c) REGISTER_LONG_CONSTANT((s), (c), CONST_CS | CONST_PERSISTENT)
  ...
  /* flags for SWFSound */
  CONSTANT("SWFSOUND_NOT_COMPRESSED",     SWFSOUND_NOT_COMPRESSED);
  CONSTANT("SWFSOUND_ADPCM_COMPRESSED",   SWFSOUND_ADPCM_COMPRESSED);
  CONSTANT("SWFSOUND_MP3_COMPRESSED",     SWFSOUND_MP3_COMPRESSED);
  CONSTANT("SWFSOUND_5KHZ",               SWFSOUND_5KHZ);
  CONSTANT("SWFSOUND_11KHZ",              SWFSOUND_11KHZ);
  CONSTANT("SWFSOUND_22KHZ",              SWFSOUND_22KHZ);
  CONSTANT("SWFSOUND_44KHZ",              SWFSOUND_44KHZ);
  CONSTANT("SWFSOUND_8BITS",              SWFSOUND_8BITS);
  CONSTANT("SWFSOUND_16BITS",             SWFSOUND_16BITS);
  CONSTANT("SWFSOUND_MONO",               SWFSOUND_MONO);
  CONSTANT("SWFSOUND_STEREO",             SWFSOUND_STEREO);

  le_swfmoviep =
    zend_register_list_destructors_ex (destroy_SWFMovie_resource, NULL,
				       "SWFMovie", module_number);
  le_swfshapep =
    zend_register_list_destructors_ex (destroy_SWFShape_resource, NULL,
				       "SWFShape", module_number);
  le_swffillp =
    zend_register_list_destructors_ex (destroy_SWFFill_resource, NULL,
				       "SWFFill", module_number);
  le_swfgradientp =
    zend_register_list_destructors_ex (destroy_SWFGradient_resource, NULL,
				       "SWFGradient", module_number);
  le_swfbitmapp =
    zend_register_list_destructors_ex (destroy_SWFBitmap_resource, NULL,
				       "SWFBitmap", module_number);
  le_swftextp =
    zend_register_list_destructors_ex (destroy_SWFText_resource, NULL,
				       "SWFText", module_number);
  le_swftextfieldp =
    zend_register_list_destructors_ex (destroy_SWFTextField_resource, NULL,
				       "SWFTextField", module_number);
  le_swffontp =
    zend_register_list_destructors_ex (destroy_SWFFont_resource, NULL,
				       "SWFFont", module_number);
  le_swfbuttonp =
    zend_register_list_destructors_ex (destroy_SWFButton_resource, NULL,
				       "SWFButton", module_number);
  le_swfmorphp =
    zend_register_list_destructors_ex (destroy_SWFMorph_resource, NULL,
				       "SWFMorph", module_number);
  le_swfspritep =
    zend_register_list_destructors_ex (destroy_SWFSprite_resource, NULL,
				       "SWFSprite", module_number);
  le_swfdisplayitemp =
    zend_register_list_destructors_ex (NULL, NULL, "SWFDisplayItem",
				       module_number);
  le_swfactionp =
    zend_register_list_destructors_ex (NULL, NULL, "SWFAction",
				       module_number);
  le_swfinputp =
    zend_register_list_destructors_ex (destroy_SWFInput_resource, NULL,
				       "SWFInput", module_number);
  le_swfsoundp =
    zend_register_list_destructors_ex (destroy_SWFSound_resource, NULL,
               "SWFSound", module_number);
  ...
  INIT_CLASS_ENTRY (gradient_class_entry, "swfgradient",
		    swfgradient_functions);
  ...
  zend_register_internal_class(&sound_class_entry);
