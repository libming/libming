...
void printDefineFont2(FILE *f, int length)
{
  ...
  name[i++] = 'b';
  name[i] = '\0';
  printf("\t$f%i = new SWF::Font(\"%s\");  # This file should exist.  \n", id, name);
...
void printDefineText(FILE *f, Blocktype type) /* type 2 allows transparency */
{
  ...
  printf("\t$s%i = new SWF::Shape();  # Empty shape for text, for now \n", id);
...
void printDoAction(FILE *f, int length)
{
  if(movieclip == 0)
  {
    printf("\t$m->add(new SWF::Action(\"\n");
    decompileAction(f, length, 0);
    printf("\"));\n");
  }
  else
  {
    printf("\t$s%i->add(new SWF::Action(\"\n", movieclip);
    decompileAction(f, length, 0);
    printf("\"));\n");
  }
}
...
void printMovieClip(FILE *f, int length)
{
  ...
  printf("\n\t### MovieClip %i ###\n", movieclip);
  printf("\t$s%i = new SWF::MovieClip();  # %i frames\n", movieclip, nFrames);
...
int main(int argc, char *argv[])
{
  ...
  printf("# Change this to your needs. If you installed ming for perl global you don't need this, i think..\n");
  ...
  printf("# I just copied it from a sample, needed to use Constants like SWFFILL_RADIAL_GRADIENT\n");
