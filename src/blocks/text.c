...
char *
SWFTextRecord_getString (SWFTextRecord record)
{
...
SWFTextRecord
SWFTextRecord_getNext (SWFTextRecord record)
{
  return record->next;
}
...
void
SWFText_setFont (SWFText text, SWFBlock font)
{
...
SWFText_addString (SWFText text, const char *string, int *advance)
{
  ...
  for (i = 0; i < l; ++i)
  {
    ...
    if (i < l - 1)
      adv +=
	SWFFont_getCharacterKern (font, (byte) string[i],
				  (byte) string[i + 1]);
...
SWFText_resolveCodes (SWFText text)
{
  ...
  while (textRecord != NULL)
  {
	...
	nGlyphBits = max (nGlyphBits,
			  SWFOutput_numBits (SWFFont_getNGlyphs
					     (textRecord->font.font) - 1));
      }
    }
