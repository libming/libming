...
static void SWFShape_writeShapeRecord (SWFShape shape, ShapeRecord record);
...
SWFShape_end (SWFShape shape)
{
  ...
  buffer[0] =
    (SWFOutput_numBits (shape->nFills) << 4) +
...
SWFFillStyle *
SWFShape_getFills (SWFShape shape)
{
  return shape->fills;
}
...
int
SWFShape_getNFills (SWFShape shape)
{
  return shape->nFills;
}
...
SWFLineStyle *
SWFShape_getLines (SWFShape shape)
{
  return shape->lines;
}
...
int
SWFShape_getNLines (SWFShape shape)
{
  return shape->nLines;
}
...
void
SWFShape_drawScaledCurveTo (SWFShape shape, int controlx, int controly,
			    int anchorx, int anchory)
{
  SWFShape_drawScaledCurve (shape, controlx - shape->xpos,
			    controly - shape->ypos, anchorx - controlx,
			    anchory - controly);
}


void
SWFShape_drawScaledCurve (SWFShape shape, int controldx, int controldy,
		       ...
		       byte r, byte g, byte b, byte a)
{
  ...
  if (width == 0)
    ...
    shape->lineWidth =
      (SWFLineStyle_getWidth (shape->lines[line - 1]) + 1) / 2;
...
SWFFillStyle
SWFShape_addGradientFillStyle (SWFShape shape,
...
SWFShape_moveScaledPenTo (SWFShape shape, int x, int y)
{
  ...
  {
    SWFRect_setBounds (SWFCharacter_getBounds (CHARACTER (shape)), x, x, y,
		       y);
  }
}
			  ...
			  SWFFont font, unsigned char c, int size)
{
  ...
  if (readBits (f, 4) != 1)	/* fill bits */
    SWF_error
      ("SWFShape_drawGlyph: bad file format (was expecting fill bits = 1)");

  if (readBits (f, 4) != 0)	/* line bits */
    SWF_error
      ("SWFShape_drawGlyph: bad file format (was expecting line bits = 0)");
  ...
  if (readBits (f, 1) != 1)	/* fill1 = 1 */
    SWF_error
      ("SWFShape_drawGlyph: bad file format (was expecting fill1 = 1)");
