...
SWFButton_addSound(SWFButton button, SWFSound sound, byte flag)
{
	if ( button->sounds == NULL )
	{
		...
		SWFCharacter_addDependency((SWFCharacter)button, (SWFBlock)button->sounds);
	}
...
void destroySWFButtonSound(SWFBlock buttonSound)
{
	sec_free((void**)&buttonSound);
}


void writeSWFButtonSoundToMethod(SWFBlock block,
																 SWFByteOutputMethod method, void *data)
{
...
int completeSWFButtonSound(SWFBlock block)
{
