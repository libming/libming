...
readSInt32 (Buffer f)
{
  return (long) readUInt8 (f) + (readUInt8 (f) << 8) + (readUInt8 (f) << 16) +
    (readUInt8 (f) << 24);
}
...
readUInt32 (Buffer f)
{
  return (unsigned long) (readUInt8 (f) + (readUInt8 (f) << 8) +
			  (readUInt8 (f) << 16) + (readUInt8 (f) << 24));
}
...
void
printDoAction (Buffer f, int length)
{
  int end;
