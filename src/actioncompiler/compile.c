...
bufferPatchLength (Buffer buffer, int back) 
{
...
bufferPatchPushLength (Buffer buffer, int len) 
{
    ...
    
  {
      ...
      (buffer->pushloc[0] & 0xff) | ((buffer->pushloc[1] & 0xff) << 8);
...
bufferWriteConstants (Buffer out) 
{
    ...
    
  {
      ...
      bufferWriteHardString (out, constants[i], strlen (constants[i]) + 1);
...
destroyBuffer (Buffer out) 
{
  sec_free ((void **) &out->buffer);
  sec_free ((void **) &out);
}
int
bufferLength (Buffer out) 
{
...
/* make sure there's enough space for bytes bytes */ 
  void
bufferCheckSize (Buffer out, int bytes) 
{
  if (bytes > out->free)
    
  {
    int new =
      BUFFER_INCREMENT * ((bytes - out->free - 1) / BUFFER_INCREMENT + 1);
...
bufferWriteData (Buffer b, byte * data, int length) 
{
...
bufferWriteBuffer (Buffer a, Buffer b) 
{
...
bufferWriteDataAndPush (Buffer a, Buffer b) 
{
  int i, pushd;
  byte * data = b->buffer;
  int length = b->pos - b->buffer;
  if (a->pushloc && (b->buffer[0] == SWFACTION_PUSHDATA)
	&& SWF_versionNum > 4)
...
bufferConcat (Buffer a, Buffer b) 
{
...
bufferWriteOp (Buffer out, int data) 
{
...
bufferWriteU8 (Buffer out, int data) 
{
...
bufferWriteS16 (Buffer out, int data) 
{
...
bufferWriteHardString (Buffer out, byte * string, int length) 
{
...
bufferWriteConstantString (Buffer out, byte * string, int length) 
{
...
bufferWriteString (Buffer out, byte * string, int length) 
{
...
bufferWriteInt (Buffer out, int i) 
{
...
bufferWriteDouble (Buffer out, double d) 
{
...
bufferWriteBoolean (Buffer out, int val) 
{
...
bufferWriteRegister (Buffer out, int num) 
{
...
bufferWriteSetRegister (Buffer out, int num) 
{
...
bufferWriteSetProperty (Buffer out, char *string) 
{
...
bufferWriteWTHITProperty (Buffer out) 
{
...
bufferWriteGetProperty (Buffer out, char *string) 
{
